...
#
#
# vi /usr/local/bin/tkjpeg
# vi /usr/local/bin/widgets
#
# usage:
#
# perl test_mechanize.pl < word_list2 > /tmp/test2.html
# cat /tmp/test2.html | htmldoc -f /tmp/list3.pdf - --webpage --size a4 --no-numbered --fontsize 18
#
#  perl test_mechanize.pl < ~/BKP/EJERCICIOS_CLAUDIO/lista3 | htmldoc -f ~/BKP/EJERCICIOS_CLAUDIO/chavo_del_ocho.pdf - --webpage --size a4 --no-numbered --fontsize 18
#
# LATEST FORMAT:
#
# perl test_mechanize.pl -pdf_name=output_files/test_wl_16 < word_lists/word_list_16
#
#

my @word_list = grep{ $_ } map { chomp $_; $_ } (<STDIN>);
