...
$conf_file ||= 'nuevo_archivo';

open FOO, $conf_file;

my @needed;
while(<FOO>) {
    my ($line_num, $id, $letter, $desc) = split( q{,}, $_ );
    chomp $desc;
    push @needed, [$line_num, $id];
}
...
$new_frame->Label( -text => 'hola hola' )->pack(@pl);
